...
function previous() {
  if (index - 1 < 0) index = imagens.length - 1;
  else index--;
  img.src = `${servidorDasImagens}/${imagens[index].arquivo}`;
  img.alt = `${servidorDasImagens}/${imagens[index].descricao}`;
}

function next() {
  if (index + 1 >= imagens.length) index = 0;
  else index++;
  img.src = `${servidorDasImagens}/${imagens[index].arquivo}`;
  img.alt = imagens[index].descricao;
}

anterior.onclick = previous;
proximo.onclick = next;
